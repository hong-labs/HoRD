...
data_root = Path(os.environ.get("HORD_TEST_DATA_ROOT", str(default_data_root)))
skeletons_dir = data_root / "skeletons"
motions_dir = data_root / "motions"


def test_skel_tree():
    skel_tree = SkeletonTree.from_mjcf(
        str(skeletons_dir / "humanoid_mimic_mod_2_noind.xml"),
        backend="pytorch",
    )
...
def test_grad():
    source_motion = SkeletonMotion.from_file(
        str(motions_dir / "JogFlatTerrain_01_ase.npy"),
        backend="pytorch",
        device="cuda:0",
    )
    source_tpose = SkeletonState.from_file(
        str(skeletons_dir / "fox_tpose.npy"),
        backend="pytorch",
        device="cuda:0",
    )

    target_tpose = SkeletonState.from_file(
        str(skeletons_dir / "flex_tpose.npy"),
        backend="pytorch",
        device="cuda:0",
    )
