...
from hord.utils.scene_lib import SceneLib
import os
from pathlib import Path
import xml.etree.ElementTree as ET
import numpy as np
from typing import Dict, List, Any, Optional, Tuple
from hord.simulator.isaaclab.utils.scene import SceneCfg
from hord.simulator.base_simulator.simulator import Simulator
from hord.simulator.base_simulator.config import (
    MarkerState,
    ControlType,
    VisualizationMarker,
    SimBodyOrdering,
...
class IsaacLabSimulator(Simulator):
    ...
    def _add_domain_randomization(self, env_ids: Optional[torch.Tensor]) -> None:
        # If env IDs are not specified, use the default configuration.
        if env_ids is None:
            return
    ...
    def _randomize_body_com(self, env_ids: torch.Tensor | None):
        if env_ids is None:
            return
        # Create distribution parameters directly on coms.device in range [-0.05, 0.05] (meters).
        com_range = {"x": (-0.02, 0.05), "y": (-0.02, 0.02), "z": (-0.005, 0.005)}
        # Find the index of torso_link in body_names.
        ...
        # IsaacLab/PhysX may require CPU indices; convert locally at call site to avoid global CPU round-trips.
    ...
    def render(self) -> None:
        ...
        if not self.headless:
            if not hasattr(self, "_perspective_view"):
                from hord.simulator.isaaclab.utils.perspective_viewer import (
                    PerspectiveViewer,
                )
