...
from hord.envs.mimic.mimic_utils import (
    dof_to_local,
    exp_tracking_reward,
)
from hord.envs.base_env.env_utils.humanoid_utils import quat_diff_norm
from hord.simulator.base_simulator.config import MarkerConfig, VisualizationMarker, MarkerState

from hord.envs.base_env.env import BaseEnv
from hord.envs.mimic.components.mimic_obs import MimicObs
from hord.envs.mimic.components.mimic_motion_manager import MimicMotionManager
from hord.envs.mimic.components.masked_mimic_obs import MaskedMimicObs
from hord.global_config import Config


class Mimic(BaseEnv):
    def _is_dr_action_mode(self):
        """Check whether DR action mode is enabled."""
    ...
    def __init__(self, config, device: torch.device, *args, **kwargs):
        ...
        # DR action mode: create collected_data motion lib for blue markers
    ...
    def create_visualization_markers(self):
        ...
        # DR action mode: add blue markers to visualize collected_data trajectory
        if self._is_dr_action_mode():
            dr_body_markers = []
            for body_name in body_names:
                if (
    ...
    def _create_dr_collected_motion_lib(self):
        """Create collected_data MotionLib instance for DR action mode."""
        import os
        from hord.utils.motion_lib import MotionLib
        from hord.utils.motion_lib_h1 import H1_MotionLib
        
        # Get motion_file parameter
        motion_file = getattr(self.config, 'motion_file', None) or getattr(self.config.motion_lib, 'motion_file', None)
        if motion_file is None:
            return None
        
        # Build collected_data path
        ...
        # Select MotionLib type based on config
    ...
    def _check_and_reload_dr_motion_lib(self):
        """Check and reload DR motion lib if the file is updated."""
        ...
        # Reload if the file has been updated
    ...
    def get_markers_state(self):
        ...
        # Update mimic markers (red markers)
        ...
        # DR action mode: update blue marker states
        if self._is_dr_action_mode():
            
            # Check and reload motion lib if file changed
            self._check_and_reload_dr_motion_lib()
            
            if self._dr_collected_motion_lib is not None:
                # Query states from collected_data motion lib
                # Ensure correct timing to avoid desynchronization
                current_time = self.motion_manager.motion_times.clone()
                
                # Clamp collected-data time range to avoid out-of-bounds
                max_time = self._dr_collected_motion_lib.get_motion_length(self.motion_manager.motion_ids)
                current_time = torch.clamp(current_time, torch.tensor(0.0, device=current_time.device), max_time - 0.001)  # avoid boundary issues
                ...
                # Blue markers should show the real collected trajectory; no forced alignment needed
                # Apply the same coordinate-system adjustments as red markers
                dr_target_pos = dr_ref_state.rigid_body_pos.clone()
                
                # Apply same respawn_offset adjustment
                dr_target_pos += self.respawn_offset_relative_to_data.clone().view(
                    self.num_envs, 1, 3
                )
                
                # Apply same terrain height adjustment
    ...
    def compute_reward(self):
        ...
        for rew_name, rew in rew_dict.items():
            ...
            # Compute std only when sample count is greater than 1
            if rew.numel() > 1:
