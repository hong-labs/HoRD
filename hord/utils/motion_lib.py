...
from hord.simulator.base_simulator.config import RobotConfig
...
# CT hack: remap phys_anim to hord for backwards compatibility of pre-existing motion files
import sys
import hord

sys.modules['phys_anim'] = hord
...
class MotionLib(DeviceDtypeModuleMixin):
    ...
    def _load_motions(self, motion_file, target_frame_rate):
        ...
        for f in range(num_sub_motions):
            ...
            # For force_retarget data already normalized to 30 FPS, applying this conversion can break timing.
